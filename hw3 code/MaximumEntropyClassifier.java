...
public class MaximumEntropyClassifier<I, F, L> implements
	...
	public static class Factory<I, F, L> implements
		...
		public double[] buildInitialWeights(IndexLinearizer indexLinearizer) {
			return DoubleArrays.constantArray(0.0,
					indexLinearizer.getNumLinearIndexes());
		}

		public IndexLinearizer buildIndexLinearizer(Encoding<F, L> encoding) {
		...
		public EncodedDatum[] encodeData(List<LabeledInstance<I, L>> data,
	...
	public static class ObjectiveFunction<F, L> implements
		...
		private Pair<Double, double[]> calculate(double[] x) {
			...
			double unregularizedObjective = 0.0;
			double[] derivatives = DoubleArrays.constantArray(0.0, dimension());
			double penalty = 0;
			int numInstances = data.length;
			int numClasses = encoding.getNumLabels();
			//System.out.println(encoding.getNumFeatures());
			for (int i=0; i<derivatives.length; i++)
				penalty += Math.pow(x[i],2);
			penalty = penalty / (2*Math.pow(sigma,2));
			
			for (EncodedDatum datum : data) {
				double[] logProb = getLogProbabilities(datum,
						x, encoding, indexLinearizer);
				int trueClass = datum.getLabelIndex();
				unregularizedObjective -= logProb[trueClass]; //negative to minimize
				
				int nActiveFeatures = datum.getNumActiveFeatures();
				
				for (int sparsifiedIndex = 0; sparsifiedIndex < nActiveFeatures; 
						sparsifiedIndex++) {
					int feature = datum.getFeatureIndex(sparsifiedIndex);
					for (int predClass = 0; predClass < numClasses; predClass++) {
						int wtIndex = indexLinearizer.getLinearIndex(feature, predClass);
						double observedCount = datum.getFeatureCount(sparsifiedIndex);
						
						double predProbability = Math.exp(logProb[predClass]);
						double expCount = predProbability * observedCount;
						
						if (predClass == trueClass)
							derivatives[wtIndex] += expCount - observedCount;
						else 
							derivatives[wtIndex] += expCount;
					}
				}
			}
				
			// Add regularization penalty to every derivative
			for (int wtIndex = 0; wtIndex < derivatives.length; wtIndex++)
				derivatives[wtIndex] += x[wtIndex] / Math.pow(sigma,2);
	
			objective = unregularizedObjective + penalty;
			//objective = unregularizedObjective;
	...
	private static <F, L> double[] getLogProbabilities(EncodedDatum datum,
		...
		for (int j=0; j<numLabels; j++) 
			normalizer += Math.exp(score[j]);
		
		normalizer = -1. * Math.log(normalizer);
		
		for (int j=0; j<numLabels; j++) 
			logProbabilities[j] = score[j] + normalizer; 
